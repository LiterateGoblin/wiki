...
Data written to an ElastiCache server is stored in the server's primary memory (DRAM) [^2]. The ElastiCache server is placed "in-between" the application and a database. When the application needs some data, it makes a request to the ElastiCache server rather than the database directly. If the data is stored on the ElastiCache server (called a cache hit) it is returned in the request. If the data could not be found on the ElastiCache server (cache miss), the request is forwarded to the database instead and returned to the requesting application while simultaneously being written to the cache.

## Redis

ElastiCache can run Redis in two modes: non-clustered and clustered mode [^2].

### Non-clustered mode

In non-clustered mode, a primary instance that can be both read and written to is supported by some number of replica instances that can only be read from (up to 5) [^2]. These instances will be placed in a subnet within an AWS Virtual Private Cloud (VPC), and can be secured by being placed in a private subnet. For additional security, a Security Group can be placed around the ElastiCache Redis instances that define which IP addresses are allowed to ingress the instances and on which ports. The instances can be distributed across subnets in different AWS Availability Zones for higher reliability. Automatic failover can be enabled on these instances - in the event of a failure of the primary instance, a read replica will be promoted to the primary instance.
